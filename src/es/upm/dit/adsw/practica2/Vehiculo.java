...
public class Vehiculo {
	...
	public List<Vector> irA(Vector destino) {
		...
		if(navegador.getMapa().estaIncluido(destino)) {
			throw new IllegalArgumentException();
		}
		
		List<Vector> list_vector = new ArrayList<Vector>();
		Tramo tramo;
		navegador.setSecuenciaMovimientos(destino);
		if(navegador.finMovimiento()==false) {
		do{
			tramo = navegador.siguienteMovimiento();
			navegador.getMapa().mueve(this, tramo);
			this.mover(tramo.hasta(),tramo.tiempo()+this.t);
			list_vector.add(tramo.hasta());
		}while(navegador.finMovimiento()==false); 
		}
		return list_vector;
	}
